...
#define DMA_CHANNEL    DMA1_Channel5 // 映射 USART1_RX
...
static void dmaInit(void)
{
    ...
    DMA_DeInit(DMA_CHANNEL); // 查看具体DMA手册找到USART占用的MDA通道
    ...
    // delay_ms(1000);
    // printf("DMA_GetCurrDataCounter: %d\n", DMA_GetCurrDataCounter(DMA_CHANNEL));
}
...
{
    DMA_Cmd(DMA_CHANNEL, DISABLE);                          // 关闭USART1 TX DMA1 所指示的通道
    DMA_SetCurrDataCounter(DMA_CHANNEL, USART_RX_BUF_SIZE); // 从新设置缓冲大小,指向数组0
    DMA_Cmd(DMA_CHANNEL, ENABLE);                           // 使能USART1 TX DMA1 所指示的通道
}
...
static void Receive_DataPack(void)
{
    ...
    /* 关闭DMA ，防止干扰 */
    DMA_Cmd(DMA_CHANNEL, DISABLE); /* 暂时关闭dma，数据尚未处理 */

    /* 获取接收到的数据长度 单位为字节*/
    buff_length = USART_RX_BUF_SIZE - DMA_GetCurrDataCounter(DMA_CHANNEL);
    ...
    // DMA_CHANNEL->CNDTR = USART_RX_BUF_SIZE;
    DMA_SetCurrDataCounter(DMA_CHANNEL, USART_RX_BUF_SIZE);
